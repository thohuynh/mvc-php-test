...
use Application\controller\Work;
...
class Application
{
    ...
    public function __construct()
    {
        ...
        $baseController = explode('/', $this->url_controller);

        if (count($baseController) == 1) {
            $directory = '';
            $controller = $baseController[0];
        } else {
            $directory = $baseController[0];
            $controller = $baseController[1];
        }

        // check for controller: does such a controller exist ?
        if (file_exists('./application/controller/' .$directory.'/'. $controller . '.php')) {

            // if so, then load this file and create this controller
            // example: if controller would be "car", then this line would translate into: $this->car = new car();
            require './application/controller/' .$directory.'/'. $controller . '.php';
    ...
    private function splitUrl()
    {
        if (isset($_GET['url'])) {
            ...
            if (isset($url[0]) && $url[0] == 'api') {
                $this->url_controller = ((isset($url[0]) && isset($url[1])) ? $url[0].'/'.$url[1] : null);
